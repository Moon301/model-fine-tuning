...
class MinioData(object):
  ...
  def set_client(self, service_name, endpoint_url, aws_access_key_id, aws_secret_access_key):
    ...
    # 연결 정보의 버킷 존재 여부 확인
    if not self.isbucket(self.bucket_name):
  ...
  def list_folders(self, prefix):
    # Minio에서 하위 폴더들을 나열합니다.(상위 폴더의 경로를 /로 끝나도록 지정해야 함.)
    response = self.s3_client.list_objects_v2(Bucket=self.bucket_name, Prefix=prefix, Delimiter='/')
    return [common_prefix['Prefix'] for common_prefix in response.get('CommonPrefixes', [])]
  ...
  def file_exists(self, file_path):
    ...
    # 만약 'Contents' 키가 있으면 해당 파일이 존재한다고 판단
